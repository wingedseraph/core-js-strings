...
function sumOfCodes(str) {
  ...
  if (typeof str === 'string' || str instanceof String) {
    for (let index = 0; index < str.length; index += 1) {
      sum += parseInt(str.charCodeAt(index), 10);
    }
    return sum;
  }
  return 0;
...
function formatTime(minutes, seconds) {
  const colon = ':';
  return (
    String(minutes).padStart(2, '0') + colon + String(seconds).padStart(2, '0')
  );
}
...
function countVowels(str) {
  return str.match(/[aeioyuAEYIOU]/g) ? str.match(/[aeioyuAEYIOU]/g).length : 0;
}
...
function isPalindrome(str) {
  const sanitizedString = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
...
function getCardId(value) {
  ...
  return ((str, val) => str.indexOf(val.replace(/[0]/gi, '')) / 2)(deck, value);
}
